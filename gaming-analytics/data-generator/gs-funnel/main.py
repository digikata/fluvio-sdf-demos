...
if __name__ == "__main__":
    ...
    # set run fluvio if RUN_FLUVIO is set to True or if topic argument is passed
    print(f"{len(sys.argv)}")
    run_fluvio = os.environ.get('FLUVIO_TOPIC', False) or len(sys.argv) >= 2

    if run_fluvio:
        from fluvio import Fluvio

        topic_name = sys.argv[1] if len(sys.argv) >= 2 else "gs-funnel"
        print(f"Running simulation and sending events to topic: {topic_name}")

        fluvio = Fluvio.connect()
        topic = fluvio.topic_producer(topic_name)
        while True:
            events = simulator.run_step()
            for event in events:
                key = f"{event['timestamp']}".encode("utf-8")
                jevent = json.dumps(event).encode("utf-8")
                topic.send(key, jevent)
            print(f"Sent {len(events)} events to topic {topic_name}")
            sleep(1)
    else:
        # Run multiple simulation steps and print to stdout
        for _ in range(5):
            events = simulator.run_step()
